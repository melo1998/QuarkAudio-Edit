...
const FAILURE_CASES = [
  // ---------- Category 1: Ambiguous phrasing (CoT Planner) ----------
  {
    ...
    src:    'assets/smartdj/whole_pipeline/badcase/001_ori.wav',
    edited: 'assets/smartdj/whole_pipeline/badcase/001_failed_output.wav',
  ...
  // ---------- Category 2: Long-tail retrieval miss (RAG Index) ----------
  {
    ...
    src:    'assets/smartdj/whole_pipeline/badcase/002_ori.wav',
    edited: 'assets/smartdj/whole_pipeline/badcase/002_failed_output.wav',
  ...
  // ---------- Category 3: Unnatural overlay (MMDiT Editor) ----------
  {
    ...
    roadmapRef: 'R4',
    title: 'Tunnel echo lacks spatial coherence',
    caption: 'A vehicle engine starting up then running idle.',
    instruction: 'Add the echo of a tunnel.',
    src:    'assets/smartdj/whole_pipeline/badcase/003_ori.wav',
    edited: 'assets/smartdj/whole_pipeline/badcase/003_failed_output.wav',
    expected: 'The engine sound gains natural tunnel reverberation with consistent early reflections, coloration from parallel-wall modes, and a decay tail matching a concrete enclosure.',
    observed: 'A generic reverb tail is appended that sounds spatially disconnected from the engine — the direct signal remains dry while the echo appears as a separate layer, lacking the characteristic frequency-dependent coloration of a real tunnel.',
    diagnosis: 'The model treats "add echo" as an event-addition operation rather than a scene-wide acoustic transformation. Without an explicit room-impulse-response prior, the MMDiT editor cannot synthesize physically plausible reverberation that is coherent with the source signal.',
    mitigation: 'R4 · Natural blending — condition MMDiT on a scene embedding (RT60 + wall-reflection pattern + band-energy profile) extracted from the source, and add a reverberation-coherence reward in Stage-2 GRPO to enforce spatial consistency between direct and reflected components.',
  },
